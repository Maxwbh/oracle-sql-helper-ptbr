...
SELECT owner, object_name, object_type, status, last_ddl_time
  FROM dba_objects
 WHERE status = 'INVALIDO'
   AND owner = 'MS_APP'  -- ← substitua pelo schema
 ORDER BY object_type, object_name;
...
BEGIN
  DBMS_UTILITY.compile_schema(
    schema      => 'MS_APP',  -- ← substitua pelo schema
...
-- Function
ALTER FUNCTION ms_app.calcular_imposto COMPILE;

-- Type (cuidado: pode invalidar dependentes)
ALTER TYPE ms_app.t_invoice COMPILE;
ALTER TYPE ms_app.t_invoice COMPILE BODY;

-- View
ALTER VIEW ms_app.v_active_invoices COMPILE;

-- Trigger
ALTER TRIGGER ms_app.trg_invoice_audit COMPILE;

-- Synonym (raramente fica inválido — geralmente alvo desapareceu)
ALTER SYNONYM ms_app.faturas COMPILE;
...
2. Privilégio revogado
   - Identificar:
     SELECT grantee, privilege, granted_role 
       FROM dba_role_privs WHERE grantee = 'MS_APP';
   - Solução: regrant
