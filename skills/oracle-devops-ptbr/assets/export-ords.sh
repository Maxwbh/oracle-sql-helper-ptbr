#!/bin/bash
# export-ords.sh — Wrapper que chama export_ords.py
# M&S do Brasil LTDA — contato@msbrasil.inf.br
# Usar diretamente: python scripts/export_ords.py
set -euo pipefail
...
exec python3 "${SCRIPT_DIR}/export_ords.py" --project-root "${PROJECT_ROOT}" "$@"
