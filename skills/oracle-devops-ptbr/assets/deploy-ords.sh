#!/bin/bash
# deploy-ords.sh — Wrapper que chama deploy_ords.py
# M&S do Brasil LTDA — contato@msbrasil.inf.br
# Usar diretamente: python scripts/deploy_ords.py --env [dev|hom|prod]
set -euo pipefail
ENVIRONMENT="${1:-dev}"
...
exec python3 "${SCRIPT_DIR}/deploy_ords.py" --env "${ENVIRONMENT}" --project-root "${PROJECT_ROOT}" "${@:2}"
