...
4. Conceder execute aos schemas que vão usar:
   GRANT EXECUTE ON logger TO ms_app;
   GRANT SELECT ON logger.logger_logs_5_min TO ms_app;
...
EXEC LOGGER.set_level('DEBUG', 'MS_APP_DEV');
...
SELECT time_stamp, logger_level, scope, text
  FROM logger.logger_logs
 WHERE client_identifier = 'MS_APP_USUARIO_123'
...
EXEC LOGGER.purge('MS_APP', '7'); -- remove logs de MS_APP > 7 dias
