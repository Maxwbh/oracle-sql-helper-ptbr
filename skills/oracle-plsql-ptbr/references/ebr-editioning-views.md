...
## Guia de adoção — quando e como começar

### Análise de contexto típico

| Fator | Favorável ao EBR | Desfavorável ao EBR |
|---|---|---|
| Tipo de aplicação | APEX crítico 24/7, REST sem janela | Sistema com janela de manutenção tolerada |
| Equipe | DBA com experiência em edições | Equipe sem treinamento em EBR |
| Schema | Estável, poucas mudanças de tabela | Muitas tabelas novas por sprint |
| Requisito de uptime | SLA rígido, alta disponibilidade | Manutenção noturna ou fim de semana aceita |

### Custos de adoção

- Refatorar tabelas existentes para usar editioning views: trabalho one-time significativo
- Cross-edition triggers durante migrações: complexidade nova para a equipe
- Disciplina de release: cada deploy vira processo de 4-6 etapas documentadas
- Treinamento DBA: necessário antes de adotar em produção

### Benefícios

- Zero downtime em deploy de PL/SQL e views (cobre a maioria das mudanças)
- Rollback rápido: basta reverter a default edition
- Testar mudanças com sessão isolada (`ALTER SESSION SET EDITION = ...`) antes de tornar default
- Conformidade com SLAs de alta disponibilidade

### Recomendação de adoção progressiva

- **Curto prazo**: manter modelo atual de janela de manutenção em schemas legados
- **Médio prazo** (6-12 meses): adotar EBR em módulos **novos** críticos — começar com 1 schema dedicado, não tentar migrar legados (custo > benefício)
- **Longo prazo**: avaliar migração completa se janela de manutenção virar restrição contratual
