...
EXEC DBMS_UTILITY.compile_schema(schema => 'APP_OWNER', compile_all => FALSE);
...
ALTER TYPE app_owner.t_payment COMPILE;
...
```sql
-- Schema inteiro
EXEC DBMS_STATS.gather_schema_stats(ownname => 'APP_OWNER', cascade => TRUE);

-- Tabela específica + todos os índices
EXEC DBMS_STATS.gather_table_stats(ownname => 'APP_OWNER',
                                    tabname => 'INVOICES',
                                    cascade => TRUE);
...
EXEC DBMS_REDEFINITION.can_redef_table('APP_OWNER', 'INVOICES');
